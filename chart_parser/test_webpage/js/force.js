...
let canvas_main = function (data) {
  // update xiaopang initialize filter
  console.log("initiate type_filter")
  window.type_filter = {};
  ...
  let begin_point = {
    x: (window_width - data.size.width * resize) / 2 - chart_margin.x,
    y: (window_height - data.size.height * resize) / 2 - chart_margin.y,
  };
  let try_draw = new draw_a_canvas(
    svg,
    new_chart_json,
    begin_point,
    (resize = resize)
...
let draw_a_canvas = function (svg, chart_json, begin_point, resize = 1) {
  ...
  let current_canvas_svg = current_canvas
    .append("svg")
  ...
  let back_board_obj = current_canvas
  ...
  current_canvas.on("click", function () {
    current_canvas.raise();
  });
  ...
  let svg_contain = current_canvas
    .append("g")
    .attr(
      "transform",
      "translate(" + chart_margin.x + "," + chart_margin.y + ")"
    );

  let svg_contain_back = "";

  if (svg_string !== "") {
    svg_contain.html(svg_string);
  ...
  let svg_array = [
    '<svg width="25px" height="25px" viewBox="0 0 18 18" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">        <g id="main" transform="translate(-663.000000, -176.000000)">            <g id="View" transform="translate(272.000000, 172.000000)">                <g id="Group-5" transform="translate(391.000000, 4.000000)">                    <rect id="Rectangle" x="0" y="0" width="18" height="18"></rect>                    <path d="M9,17 C13.418278,17 17,13.418278 17,9 C17,4.581722 13.418278,1 9,1 C4.581722,1 1,4.581722 1,9 C1,13.418278 4.581722,17 9,17 Z M4.99917209,5.00124609 L13,13 M5,13 L13.0019084,5.00313936" id="Oval-2" stroke="#797979"></path>                </g>            </g>        </g>    </g></svg>',
    ...
    .attr(
      ...
      (d, i) =>
        `translate(${button_left},${
          button_top + i * (button_height + button_interval)
        })`
    );

  let current_icon = current_button
    .append("g")
    // .attr('href', d => server_position + "image/" + d + "_icon.svg")
    .html((d, i) => svg_array[i]);

  current_icon.attr("width", button_width).attr("height", button_height);
  ...
  function copy_current_canvas() {
    let new_begin_point = {
      x: plot_area.x + plot_area.width + 50,
      y: plot_area.y,
    };
    new_chart_json = JSON.parse(JSON.stringify(chart_json));
    let new_canvas_object = new draw_a_canvas(
      svg,
      new_chart_json,
      new_begin_point,
      (resize = 1)
  ...
  function drag_border_start(e, d) {
    ...
    current_canvas_object.current_canvas_id = _chart_object.indexOf(
  ...
  function drag_border(e, d) {
    let current_point = get_source_point(e);
    new_position = {
      x: current_point.pageX - begin_drag.x + begin_begin_point.x,
      y: current_point.pageY - begin_drag.y + begin_begin_point.y,
    };
  ...
  function drag_border_end(e, d) {
    ...
    new_position = find_near_correct_place(
      window_width,
      window_height,
      chart_json.size.width,
      chart_json.size.height,
      old_position,
      (current_obj = current_canvas_object.current_canvas_id)
    );
  ...
  let content_group = current_canvas
    .append("g")
    .attr(
      "transform",
      "translate(" + chart_margin.x + "," + chart_margin.y + ")"
  ...
  let text_canvas = content_group
    .append("g")
    // .attr('transform', 'translate(' + chart_margin.x + "," + chart_margin.y + ")")
    .attr("id", "text_canvas");

  let force_group = current_canvas
    .append("g")
  ...
  this.draw_new_coordsys = function (coordinate_data) {
    content_group.select("#coordinate_0").remove();
    chart_json.CoordSys = [coordinate_data];
    ...
    coor_sys_list = chart_json.CoordSys.map(function (coordinate_data) {
      ...
      let current_coord_sys = new draw_a_coordinate(
        svg,
        content_group,
        text_canvas,
        coordinate_data,
  ...
  this.draw_new_coordsys(chart_json.CoordSys[0]);

  this.restart_all_simulations = function () {
    this.CoordSys.forEach((coordinate_object) =>
      coordinate_object.restart_all_simulations()
    );
  };
...
draw_a_canvas.prototype.update_all_handle = function () {
  ...
  this.force_handle_list.forEach((force_handle) =>
    force_handle.redraw_force_handle()
  );
  // To Do
};
...
draw_a_canvas.prototype.add_brush = function (canvas) {
  let edge = 10;
  let current_canvas_object = this;
  let brush_visual_obj = d3
    .brush()
    .on("end", brush_visual_obj_end)
    .extent([
      [edge, edge],
      [
        this.plot_area.width - chart_margin.x * 2 - edge,
        this.plot_area.height - chart_margin.y * 2 - edge,
      ],
    ]);
  ...
  function brush_visual_obj_end(e) {
    ...
    for (let current_coordinate_object of current_canvas_object.CoordSys) {
      ...
      if (e.selection === null) {
        ...
        visual_object.forEach((d) => (d.selected = true));
        ...
        visual_object.forEach(function (d) {
          if (!d.activate) {
            ...
            d.control_point.forEach(
              (pid) => (control_point[pid].selected = false)
            );
            return;
          }
          let selected = false;
          for (let pid of d.control_point) {
            ...
            if (
              current_point.show_x >= selection[0][0] &&
    ...
    current_canvas_object.update_highlight();
    current_canvas_object.update_all_handle();
  }
};

draw_a_canvas.prototype.update_highlight = function () {
  this.CoordSys.forEach(function (coord) {
    coord.link.classed("deselected", (d) => !d.selected);
    coord.circle_object.classed("deselected", (d) => !d.selected);
  });
};
...
draw_a_canvas.prototype.draw_axis = function (current_canvas, coor_sys_list) {
  ...
  let axis_canvas = current_canvas
    .append("g")
    .attr("id", "axis_canvas")
  ...
  let x_axis_object_list = this.chart_json.axis.x.map(function (d) {
    if (d.use_num > 0) return new draw_axis(axis_canvas, d, main_canvas_object);
    else return null;
  });
...
draw_a_canvas.prototype.deactivate_visual_object_group = function (
  vid_group,
  coordinate_id
) {
...
draw_a_canvas.prototype.activate_visual_object_group = function (
  vid_group,
  coordinate_id
) {
...
draw_a_canvas.prototype.add_force_layer = function (force_canvas) {
  ...
  // the force
  function get_force_position_all_coordinate(
    force_info,
    type = "x",
    force_type = "should"
  ) {
    ...
    chart_json.CoordSys.forEach(function (coordinate_data) {
      ...
      for (
        let i = 0, control_point_num = control_point.length;
        i < control_point_num;
        i++
      ) {
        let node = control_point[i];
        if (!node.hasOwnProperty(attr_name)) continue;
    ...
    for (const [key, value] of Object.entries(gravity_power)) {
      let current_force = {
        ...
        control_point: value,
        type: type,
        min: Math.min(...value.map((d) => d[another_attr])),
        max: Math.max(...value.map((d) => d[another_attr])),
      };
      current_force.visual_object = new Array(
        ...new Set(current_force.control_point.map((d) => d.obj_id))
      );
  ...
  let draw_gravity_power = function (
    canvas,
    type = "x",
    force_type = "should",
    edge = 10
    ...
    let gravity_canvas = canvas.append("g").attr("class", force_name);
    ...
    let redraw_gravity_power = function () {
      ...
      let gravity_count = gravity_power
        .map((d) => d.control_point.length)
        .reduce((a, b) => a + b, 0);
      ...
      g_base = gravity_canvas
        .selectAll("." + force_name)
        ...
        .style("display", function (d, i) {
          if (d.control_point.length / gravity_count < 0.1) return "none";
          if (i > 20) return "none";
          return null;
        });

      let chart_real_width =
        current_canvas_object.plot_area.width - chart_margin.x * 2;
      let chart_real_height =
        current_canvas_object.plot_area.height - chart_margin.y * 2;
    ...
    function dragend(e, d) {
      ...
      for (let node of d.control_point) node[force_name] = d.value;
      update_line_position(d, d3.select(this));
      update_all_line_position();
    }

    let update_all_line_position = function () {
      g_base.each(function (d) {
        update_line_position(d, d3.select(this));
      });
    ...
    let update_line_position = function (d, current_object) {
      ...
      if (d.type === "x" || d.type === "cx") {
        ...
        current_object.attr("transform", "translate(" + start_point.x + ",0)");
      } else {
        start_point = xy2screen(0, d.value, main_transform);
        // end_point = xy2screen(d.min - edge, d.value, main_transform)

        current_object.attr("transform", "translate(0," + start_point.y + ")");
      }
    };
  ...
  this.force_button_list.push(
    new add_force_line_new(
      force_canvas,
      this,
      (force_type = "smaller"),
      (attr_type = "y"),
  ...
  this.force_button_list.push(
    new add_force_collision(
      force_canvas,
      this,
      (force_type = "collision"),
      (index = 6)
    )
  );
  this.force_button_list.push(
    new add_color_axis(force_canvas, this, (index = 7))
  );
  ...
  this.click_force_button = function () {
    this.force_group.style(
      "display",
      this.force_group.style("display") === "none" ? null : "none"
    );
    this.update_all_handle();
  };
};

function draw_force_line(
  g_base,
  chart_real_width,
  chart_real_height,
  type = "x",
  ...
  if (force_type === "should") {
    ...
    if (type === "x") {
      ...
      if (force_type === "larger") {
        force_line_rect.attr("x", -force_line_width);
        force_line_border.attr("x", -1.5);
        force_line_border_thin.attr("x", -force_line_width);
        force_line_back_board.attr("x", -force_line_width);
      }
    } else {
      force_line_rect
        .attr("fill", "url(#strip_pattern)")
      ...
      if (force_type === "larger") {
        force_line_rect.attr("y", -force_line_width);
        force_line_border.attr("y", -1.5);
        force_line_border_thin.attr("y", -force_line_width);
        force_line_back_board.attr("y", -force_line_width);
      }
    }
  }
}
...
function add_force_line_new(
  ...
  button = force_layer_canvas
    ...
    .attr(
      ...
      "translate(" +
        chart_real_width +
        ", " +
        (button_height + button_interval) * index +
        ")"
    );
  ...
  let button_icon = button
    ...
    .attr("transform", function () {
      ...
      let y_position = 0;
      if (attr_type === "x") {
        if (force_type === "larger") x_position = button_width * bias_rate;
        else if (force_type === "smaller")
          x_position = button_width * (1 - bias_rate);
        else x_position = button_width * 0.5;
      } else {
        if (force_type === "larger") y_position = button_height * bias_rate;
        else if (force_type === "smaller")
          y_position = button_height * (1 - bias_rate);
        else y_position = button_height * 0.5;
      }
  ...
  force_area = force_layer_canvas.append("g").style("display", "none");

  force_line = force_area.append("g").style("pointer-events", "none");
  ...
  let force_rect = force_area
    ...
    .on("click", function (e, d) {
      ...
      chart_json.CoordSys.forEach(function (coordinate_data) {
        let value = screen2xy(chart_x, chart_y, coordinate_data.transform)[
          attr_type
        ];
  ...
  this.update_force_button = function () {
    ...
    force_rect
      .attr("width", chart_real_width)
      .attr("height", chart_real_height);
    force_line.selectAll("rect").each(function (d) {
      if (attr_type === "x") d3.select(this).attr("height", chart_real_height);
      else d3.select(this).attr("width", chart_real_width);
    });
  };
}
...
function add_color_axis(force_group, main_canvas_object, index = 0) {
  ...
  let axis_line = force_area.append("g").style("pointer-events", "none");

  let axis_link = axis_line
    .append("line")
    .attr("stroke", "#b3cde3")
    .attr("stroke-width", 3);

  let axis_begin = axis_line
    .append("circle")
    .attr("r", 10)
    .attr("fill", "#b3cde3");

  let axis_end = axis_line
    .append("circle")
    .attr("r", 10)
    .attr("fill", "#b3cde3");
  ...
  let choice_contain = force_area.append("g").attr("display", "none");
  ...
  let draw_line_drag = d3
    ...
    .on("start", function (e, d) {
      ...
      axis_begin.attr("cx", chart_x).attr("cy", chart_y);

      axis_end.attr("cx", chart_x).attr("cy", chart_y);
    })
    .on("drag", function (e, d) {
      ...
      if (
        Math.abs(chart_x - begin_position.x) >
        Math.abs(chart_y - begin_position.y)
      ) {
      ...
      axis_link.attr("x2", end_position.x).attr("y2", end_position.y);
      axis_end.attr("cx", end_position.x).attr("cy", end_position.y);
    })
    .on("end", function (e, d) {
    ...
    main_canvas_object.CoordSys.forEach(function (coordinate_object) {
      ...
      visual_object.forEach(function (d) {
        if (d.activate && d.selected && d.hasOwnProperty(attr_type)) {
          ...
          if (!(current_attr in attr_group)) attr_group[current_attr] = [];
  ...
  function update_constraints(attr_type = "fill") {
    ...
    attr_group.forEach(function (d, i) {
      ...
      if (attr_type === "radius")
        current_value =
          (((end_value - begin_value) / main_canvas_object.global_resize) *
            (parseFloat(d.attr) - min_attr)) /
            (max_attr - min_attr) +
          begin_value;
      if (attr_type === "fill")
        current_value =
...
function add_force_collision(
  ...
  let force_name = force_type + "_" + attr_type;
  const force_layer_canvas = force_canvas.append("g").attr("id", force_type);
  ...
  button.on("click", function (d) {
    main_canvas_object.CoordSys.forEach(function (d) {
      d.control_point.forEach(function (d) {
        if (d.activate && d.selected && d.radius > 0) d.collide = !d.collide;
      });
