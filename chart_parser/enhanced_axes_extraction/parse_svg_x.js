...
const directoryName = "input_data"; 
...
(async () => {
  ...
  const progressBar = new SingleBar({
    format: '3. SVG Parsing Progress {percentage}%|{bar}|{value}/{total} [ETA: {eta}]',
    barCompleteChar: '█',
    barIncompleteChar: ' ',
    hideCursor: true
  });

  // Start progress bar with total number of SVG files
  progressBar.start(files.length, 0);
  let i = 0

  for (const file of files) {
    const filePath = `file://${directoryPath}/${file}`;
    ...
    fs.writeFileSync(
      `${outputDirectory}/${file.split('.')[0]}.txt`,
      simVectorContent
    );
